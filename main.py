# ==============================
#  grid/live bot – Bitvavo (fee-aware)
# ==============================
...
# >>> Nieuw: fee-bewuste verkoop
FEE_PCT         = float(os.environ.get("FEE_PCT", "0.0015"))   # 0,15% per order
MIN_PROFIT_PCT  = float(os.environ.get("MIN_PROFIT_PCT", "0.0025"))  # 0,25% extra
REQUIRED_EDGE   = 2*FEE_PCT + MIN_PROFIT_PCT                   # rondtrip + netto winst
...
BASE, QUOTE = SYMBOL.split("/")  # bv. ETH / EUR
...
print(
    ...
    f"| SL={STOP_LOSS*100:.1f}% | TP={TAKE_PROFIT*100:.1f}% | Edge≥{REQUIRED_EDGE*100:.2f}% (fees incl.)"
)
bot_logger.info(
    "START | symbol=%s tf=%s live=%s eur_per_trade=%.2f sl=%.3f tp=%.3f edge=%.4f",
    SYMBOL, TIMEFRAME, LIVE, EUR_PER_TRADE, STOP_LOSS, TAKE_PROFIT, REQUIRED_EDGE
)
...
# ========== Trade state ==========
avg_entry_price: float | None = None   # gewogen gemiddelde instap
position_amt: float = 0.0              # netto hoeveelheid gekocht door de bot (boven start)
EPS = 1e-8
...
def update_avg_entry(curr_avg: float | None, curr_amt: float, add_amt: float, buy_px: float) -> float:
    """Gewogen gemiddelde instap na een bij-kauf."""
    if curr_avg is None or curr_amt <= EPS:
        return buy_px
    return (curr_avg * curr_amt + buy_px * add_amt) / (curr_amt + add_amt)
...
while True:
    try:
        ...
        # -------- SL/TP + Trailing (alleen als winst of SL/TP voorwaarden) --------
        if avg_entry_price and position_amt > EPS:
            change_pct = (px - avg_entry_price) / avg_entry_price

            # Trailing activeren pas NA required edge (winst na fees)
            if not trail_active and change_pct >= max(TRAIL_TRIGGER, REQUIRED_EDGE):
                trail_active = True
                trail_peak  = px
                start_lock  = avg_entry_price * (1 + max(TRAIL_START, REQUIRED_EDGE))
        ...
        if sig == "BUY":
            ...
            if eur_room <= 0:
                ...
                # positie & gemiddelde instap bijwerken
                avg_entry_price = update_avg_entry(avg_entry_price, position_amt, filled, px)
                position_amt += filled
                reset_trailing()  # nieuwe build-up -> trailing opnieuw

                msg = f"BUY ~€{spend_eur:.2f} {BASE} (+{filled:.6f}) @ €{px:.6f} | avg_entry=€{avg_entry_price:.6f}"
                print("[LIVE]", msg); bot_logger.info(msg)
        ...
        # --------------------------- SELL --------------------------
            ...
            if position_amt > EPS and base_for_bot > 0:
                # >>> verkoop alleen als netto winst ≥ REQUIRED_EDGE
                if avg_entry_price is not None:
                    gain = (px - avg_entry_price) / avg_entry_price
                else:
                    gain = 0.0

                if gain + 1e-12 >= REQUIRED_EDGE:
                    ...
                    if to_sell >= MIN_BASE and (to_sell * px) >= max(MIN_QUOTE, 0.50):
                        ...
                        with_retry(ex.create_order, SYMBOL, "market", "sell", to_sell, None, params)
                        position_amt = max(0.0, position_amt - to_sell)
                        msg = f"SIGNAL SELL {to_sell} {BASE} @ €{px:.6f} | gain={gain*100:.2f}% (edge≥{REQUIRED_EDGE*100:.2f}%)"
                        print("[LIVE]", msg); bot_logger.info(msg)
                    ...
                    print(f"[EDGE] Te weinig winst na fees: gain={gain*100:.2f}% < edge {REQUIRED_EDGE*100:.2f}%. HOLD.")
            else:
