...
# ENV (exact):
# API_KEY, API_SECRET
# OPERATOR_ID (optioneel; numeriek zonder quotes; alleen zetten als Bitvavo dat vereist)
# EXCHANGE=bitvavo, DATA_DIR=data, overige defaults op wens
...
def _guard_operator_error(exc: Exception):
    ...
    if "operatorid" in msg and "required" in msg:
        print("Bitvavo meldt: operatorId vereist voor deze API-key. "
              "Gebruik een normale key of zet een geldige numerieke OPERATOR_ID.", flush=True)
        sys.exit(1)
...
def make_exchange():
    ...
    try:
        ...
        _guard_operator_error(e); raise
    return ex

def fetch_mid(ex, pair: str) -> float:
    t = ex.fetch_ticker(pair)
    bid = to_num(t.get("bid"),0.0); ask = to_num(t.get("ask"),0.0)
...
def _op_params(extra: dict | None = None) -> dict:
    if OPERATOR_ID is None: return extra or {}
    base = {"operatorId": OPERATOR_ID}
...
def process_pair(ex, pair: str, state, out: List[str]):
    ...
    if px <= 0:
        out.append(f"[{pair}] geen prijs, skip."); return
        ...
        if crossed:
            ...
            for _ in crossed:
                ...
                if freeE < (BUY_FREE_EUR_MIN + MIN_CASH_BUFFER_EUR):
                    out.append(f"[{pair}] BUY skip: policy free_EUR<€{BUY_FREE_EUR_MIN:.2f} (buffer €{MIN_CASH_BUFFER_EUR:.2f})."); continue
                if room <= 0:
                    out.append(f"[{pair}] BUY skip: cap bereikt (cap=€{cap_now(port):.2f})."); continue
                ...
                if cost <= 0:
                    ...
                    out.append(f"[{pair}] BUY skip: vrije EUR te laag (free≈€{freeE:.2f}, nodig≥€{min_quote+MIN_CASH_BUFFER_EUR:.2f}, tekort≈€{need:.2f})."); continue
    ...
    while True:
        ...
        try:
            ...
            if (time.time() - last_summary) >= LOG_SUMMARY_SEC:
                ...
                print(f"[SUMMARY] total_eq=€{cash+live_inv:.2f} | cash=€{cash:.2f} | free_EUR=€{cash:.2f} | invested_cost=€{invested_cost:.2f} | live_inv=€{live_inv:.2f} | cap_now=€{cap:.2f} | pnl_realized=€{pnl:.2f} | buys={'ON' if ALLOW_BUYS else 'OFF'}", flush=True)
