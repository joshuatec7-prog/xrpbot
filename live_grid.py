# live_grid.py — Bitvavo LIVE grid bot met harde minima + sell-safety
# - Baseline-protect, cost-cap & live-cap
# - BUY forceert min_quote/min_base (met overrides)
# - SELL wacht op best bid met extra veiligheidsmarge
...
def bot_inventory_value_eur_from_exchange(ex, state, pairs) -> float:
    """Waarde van door-bot-beheerde vrije base-balances (baseline gerespecteerd)."""
    try:
...
SELL_SAFETY_PCT = float(os.getenv("SELL_SAFETY_PCT", "0.010"))  # +1% boven winst/fee drempel
...
def try_grid_live(ex, pair, price_now, price_prev, state, grid, pairs) -> List[str]:
    ...
    if price_prev is not None and price_now < price_prev:
        ...
        for _ in crossed:
            ...
            logs.append(
                f"{COL_C}[{pair}] BUY {qty:.8f} @ €{avg:.6f} | req≈€{cost:.2f} | exec≈€{executed:.2f} | fee≈€{fee:.2f} | cash=€{port['cash_eur']:.2f}{COL_RESET}"
            )
