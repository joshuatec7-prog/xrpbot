# paper_grid.py — Multi-coin Paper Grid Bot (agressief/Scenario C)
# - Lees COINS & WEIGHTS uit ENV (5 coins standaard)
# - Gridband d.m.v. 30d 1h P10–P90 (fallback median ± BAND_PCT)
# - Virtuele buys/sells (paper), fees & realized PnL
...
# Agressiever: vergroot/verklein order-omvang per grid-event
ORDER_SIZE_FACTOR     = float(os.getenv("ORDER_SIZE_FACTOR", "2.0"))

# Veiligheid: minimale vrije buffer & minimale trade
...
def normalize_weights(pairs: List[str], weights_csv: str) -> Dict[str, float]:
    """
    weights_csv: 'BTC/EUR:0.35,ETH/EUR:0.3,...'
    """
    d: Dict[str, float] = {}
    if weights_csv:
        for item in [x.strip() for x in weights_csv.split(",") if x.strip()]:
            if ":" in item:
                ...
                try:
                    ...
                    pass
    # reduce tot alleen opgegeven pairs
...
def try_fill_grid(
    pair: str, price_now: float, price_prev: float,
    grid: dict, port: dict, ex
) -> List[str]:
    logs = []
    levels = grid["levels"]
    ...
    if price_prev is not None and price_now < price_prev:
        ...
        for L in crossed:
            # buffer & minimum
            if port["portfolio_eur"] - MIN_CASH_BUFFER_EUR < order_eur:
